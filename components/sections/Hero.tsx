...
export function Hero() {
  ...
  return (
    <section
      id="top"
      className="relative flex min-h-[100svh] items-center overflow-hidden px-4 pt-20 pb-8 sm:px-6 md:px-8 lg:pt-24"
    >
      <Clay3DAsset
        variant="workspace"
        delay="short"
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1.1fr)_minmax(20rem,0.9fr)] lg:items-center">
          ...
          >
            ...
            <motion.div variants={fadeInUp} className="mt-6 grid gap-3 sm:grid-cols-3">
              {statCards.map((stat) => (
                <div key={stat.label} className="clay-card px-4 py-4">
              ...
              {quickLinks.map((item) => {
                ...
                return (
                  <a
                    key={item.href}
                    href={item.href}
                    className="clay-inset flex items-center gap-3 px-4 py-3 transition-transform duration-200 hover:-translate-y-0.5"
                  >
